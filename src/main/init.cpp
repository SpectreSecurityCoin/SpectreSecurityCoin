...
void Shutdown()
{
    ...
    RenameThread("SpectreSecurityCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: SpectreSecurityCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: SpectreSecurityCoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 13338)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 13338)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeSpectreSecurityCoinamount=<n> " + _("Keep N SpectreSecurityCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SpectreSecurityCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SpectreSecurityCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SpectreSecurityCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "SpectreSecurityCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SpectreSecurityCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SpectreSecurityCoin to complete") << "\n";
    ...
    nAnonymizeSpectreSecurityCoinAmount = GetArg("-anonymizeSpectreSecurityCoinamount", 0);
    if(nAnonymizeSpectreSecurityCoinAmount > 999999) nAnonymizeSpectreSecurityCoinAmount = 999999;
    if(nAnonymizeSpectreSecurityCoinAmount < 2) nAnonymizeSpectreSecurityCoinAmount = 2;
    ...
    LogPrintf("Anonymize SpectreSecurityCoin Amount %d\n", nAnonymizeSpectreSecurityCoinAmount);
