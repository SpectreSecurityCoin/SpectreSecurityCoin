...
unsigned int nStakeMaxAge = 48 * 24 * 60 * 60 * 2 * 15; // 192 Days.
...
const string strMessageMagic = "SpectreSecurityCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight > 1 && nHeight <= 11) {
        nSubsidy = 12500000LL * COIN;
    }
    else if (nHeight > 11 && nHeight <= 50000) {
        nSubsidy = 5 * COIN;
    }
    else if (nHeight > 50000 && nHeight <= 125000) {
        nSubsidy = 10 * COIN;
    }
    else if (nHeight > 125000 && nHeight <= 250000) {
        nSubsidy = 15 * COIN;
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 > 350000 && pindexBest->nHeight+1 <= 660000)  {
        nSubsidy = 100 * COIN;
    }
    else if (pindexBest->nHeight+1 > 660000 && pindexBest->nHeight+1 <= 5282500)  {
        nSubsidy = 25 * COIN;
    }
...
{
    RenameThread("SpectreSecurityCoin-loadblk");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if ((pfrom->nVersion < MIN_PEER_PROTO_VERSION) || (nBestHeight >= 2 && pfrom->nVersion < MIN_PEER_PROTO_VERSION_FORK1))
        {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
	...
	ret = 0;
        ...
        ret = blockValue * 5 / 10;
    } else if (nHeight >= 250000 && nHeight <= 350000) {
        ret = blockValue * 6 / 10;
