...
//
// Bootup the masternode, look for a 25000 XSPC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(Params().NetworkID() == CChainParams::MAIN) {
            if(service.GetPort() != 13338) {
                notCapableReason = "Invalid port: " + boost::lexical_cast<string>(service.GetPort()) + " - only 13338 is supported on mainnet.";
                status = MASTERNODE_NOT_CAPABLE;
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason.c_str());
                return;
            }
        } else if(service.GetPort() == 13338) {
            notCapableReason = "Invalid port: " + boost::lexical_cast<string>(service.GetPort()) + " - 13338 is only supported on mainnet.";
        }
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in SpectreSecurityCoin.conf */
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string txHash, std::string strOutputIndex, std::string strDonationAddress, std::string strDonationPercentage, std::string& errorMessage) {
    ...
    if (GetTransaction(outpoint.hash, tx21, hashi))
    {
        BOOST_FOREACH(CTxOut value, tx21.vout)
        {
            ...
            {
                LogPrintf("ActiveMasternode::Register - Duplicate Collateral Address\n");
                return false;
            }
            else
            {
...
{
    CSpectreSecurityCoinAddress address(collateralAddress);
