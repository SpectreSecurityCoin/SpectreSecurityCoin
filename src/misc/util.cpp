...
int nAnonymizeSpectreSecurityCoinAmount = 1000;
...
#else
    const char* pszModule = "SpectreSecurityCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SpectreSecurityCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SpectreSecurityCoin
    // Mac: ~/Library/Application Support/SpectreSecurityCoin
    // Unix: ~/.SpectreSecurityCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SpectreSecurityCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "SpectreSecurityCoin";
#else
    // Unix
    return pathRet / ".SpectreSecurityCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "SpectreSecurityCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "SpectreSecurityCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong SpectreSecurityCoin will not work properly.");
