...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSpectreSecurityCoinAmount"))
        settings.setValue("nAnonymizeSpectreSecurityCoinAmount", 1000);
    nAnonymizeSpectreSecurityCoinAmount = settings.value("nAnonymizeSpectreSecurityCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSpectreSecurityCoinAmount"))
        SoftSetArg("-anonymizeSpectreSecurityCoinamount", settings.value("nAnonymizeSpectreSecurityCoinAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSpectreSecurityCoinAmount:
            nAnonymizeSpectreSecurityCoinAmount = value.toInt();
            settings.setValue("nAnonymizeSpectreSecurityCoinAmount", nAnonymizeSpectreSecurityCoinAmount);
            emit AnonymizeSpectreSecurityCoinAmountChanged(nAnonymizeSpectreSecurityCoinAmount);
            break;
        default:
            break;
        }
