...
public:
                      ...
                      const QModelIndex &index ) const
    {
        ...
        mainRect.moveLeft(ICON_OFFSET - 25);
        ...
        //painter->setPen(fUseDefaultTheme ? QColor(255, 255, 255) : foreground);
        painter->setPen(fUseDefaultTheme ? QColor(254, 254, 254) : foreground);
        QRect boundingRect;
        ...
        painter->setPen(fUseDefaultTheme ? QColor(41,44,48) : foreground);
        ...
        painter->setPen(fUseDefaultTheme ? QColor(96, 101, 110) : option.palette.color(QPalette::Text));
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSpectreSecurityCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSpectreSecurityCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(nMaxToAnonymize > nAnonymizeSpectreSecurityCoinAmount*COIN) nMaxToAnonymize = nAnonymizeSpectreSecurityCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSpectreSecurityCoinAmount * COIN) {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeSpectreSecurityCoinAmount == 0){
